...
void draw() {
  ...
  delay(500);
  ////generate.drawOutWinkyFace();
  ////generate.drawOutSmileyFaceWithClosedMouth();
  ////generate.drawOutSmileyFaceWithOpenedMouthAndClosedEyes();
  //if(select==0) {
  generate.drawInterchangeableEmoji();
  //}
  //if(select==1) {
  //generate.drawRandomThreeDotsAnimation();
  ////}
  //////delay(500);
  ////if(select==2) {
  //generate.drawRandomEyesAnimations();
  //}
}

void countUp(int x, int y) {
  if (freePlay.state[x][y] < 8) 
  {
    freePlay.state[x][y]++; 
  } else {
