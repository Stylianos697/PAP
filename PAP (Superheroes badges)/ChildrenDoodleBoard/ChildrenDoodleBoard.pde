...
void draw() {
  ...
  //delay(350);
  //delay(500);
  generate.drawAnimationOf4Quadrants();
  //generate.drawRandomPoppingOffCircleBadgeAnimation();
  //generate.drawRandomAnimationBetweenIronSpiderManAndCaptainAmerica();
}

void countUp(int x, int y) {
  if(freePlay.state[x][y] < 4) 
  {
    freePlay.state[x][y]++; 
  } else {
